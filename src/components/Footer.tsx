...
const Footer = () => {
  ...
  useEffect(() => {
    const fetchFooter = async () => {
      try {
        const res = await client.queries.footer({ relativePath: "footer.md" });
  ...
  // Default/Fallback data (matches the original hardcoded content)
  const contactInfo = data?.contactInfo || [
    { label: "Morning Pick-Ups and Drop-Offs", value: "7:30am to 9:30am" },
    { label: "Evening Pick-Ups and Drop-offs", value: "5:00pm to 7:00pm" },
    { label: "Admissions Office Hours", value: "9:00am to 5:00pm" }
  ];
  ...
  const linkColumns = data?.linkColumns || [
    {
      ...
      links: [
        { label: "Home", url: "/" },
        { label: "Contact Us", url: "https://pawhausacademy.portal.gingrapp.com/#/public/login" }
      ]
    },
    {
      title: "Services",
      links: [
        ...
        { label: "Day Training", url: "/day-training" },
        { label: "Board & Train", url: "/board-train" }
      ]
    }
  ];

  const social = data?.social || {
    facebook: "https://www.facebook.com/profile.php?id=61575107867319",
    instagram: "https://www.instagram.com/pawhausacademy/",
    yelp: "https://www.yelp.com/biz/pawhaus-academy-chatsworth"
  };
    ...
    <footer className="bg-gray-900 text-gray-300 py-12 bg-pawhaus-dark">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8  pb-8">
          ...
          <div className="space-y-4">
            {linkColumns.map((col: any, index: number) => (
              <div key={index}>
                <h3 className="text-white text-lg font-bold uppercase tracking-wider font-urbanist mb-2">
                  {col.title}
                </h3>
                <ul className="space-y-2 pb-4">
                  {col.links.map((link: any, lIndex: number) => (
                    <li key={lIndex}>
                      {link.url.startsWith("http") ? (
                        <a href={link.url} className="hover:text-white transition-colors" target="_blank" rel="noopener noreferrer">{link.label}</a>
                      ) : (
